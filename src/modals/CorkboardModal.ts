...
import { sanitizeFileName, extractSynopsisAndTags, DRAFT_FILENAME, TEMPLATES_DIR, TEMPLATES_FILENAME, getColorById, generateSceneId, SCENE_COLORS, createIconButton, getManuscriptFiles, parseUniversalScenes, parseContent } from '../utils';
...
export class CorkboardModal extends Modal {
    ...
    buildColumnDOM(container: HTMLElement | DocumentFragment, colTitle: string, filePath: string | null, scenes: CorkboardScene[], insertBeforeEl?: HTMLElement) {
        ...
        btnAddScene.onclick = () => {
            new SceneCreateModal(this.app, "Create New Scene", "", (result, colorId) => {
                const newScene = { id: generateSceneId(), title: result, colorId: colorId, meta: [] as string[], isNew: true };
    ...
    async renderGlobalCards(container: HTMLElement) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            for (const card of parsedData.cards) {
                ...
                if (!safeKey) {
                    const count = titleCollisionCount.get(card.key) || 0;
                    safeKey = count === 0 ? card.key : `${card.key}_${count}`;
                    titleCollisionCount.set(card.key, count + 1);
                }
    ...
    async openScenePanel(cardEl: HTMLElement, scene: CorkboardScene) {
        ...
        if (!currentText && cardEl.dataset.isNew === "true") {
            ...
            if (TEMPLATES_DIR) {
                const tplFile = this.app.vault.getAbstractFileByPath(`${this.plugin.settings.bookFolderPath}/${TEMPLATES_DIR}/${TEMPLATES_FILENAME}`);
